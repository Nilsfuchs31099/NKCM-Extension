...
var Z   		${Z_t}$ (long_name='TFP'),  
    ...
    gZ_trend    ${g^{trend}_{z,t}}$ (long_name='Baseline TFP growth trend'),
    L_nc        ${L^{nc}_t}$ (long_name='Population, climate-free reference'),
    gL_nc       ${g^{nc}_{l,t}}$ (long_name='Population growth, climate-free reference');
...
parameters   
	...
	% Now the climate population interaction parameter
    phi_m       $\phi_{m}$                  (long_name='Climate-mortality sensitivity')
    M_ref       $m_{ref}$                   (long_name='Reference carbon stock for mortality wedge (GtC)')
    % Now the climate technology interaction parameter
    psi_l       $\psi_{1}$                  (long_name='Semi-endogenous growth: Population growth elasticity of TFP growth')
    psi_m       $\psi_{m}$                  (long_name='Direct climate effect on TFP growth in a rate of change form')
	;
...
/* Demographic-climate extension */
M_ref       = M0;           % = 719.94 GtC: wedge measured relative to the 1985 stock
phi_m       = 5.9e-8;      % Baseline --> 0 to reproduce SSV exactly use 5.9e-8 for linear and 5.3e-11 for quadratic

/* Technology-growth extension */
psi_l       = 0.32;            % Jones semi-endogenous link; O reproduces SSV exactly 0.32-0.24 bound
psi_m       = 3e-5;            % direct climate effect; 0 reproduces SSV exactly, use 3e-5
...
model;
	...
	Z 		= Z(-1)*(1+gZ(-1));
    gZ_trend = (1-GZ1)*gZ_trend(-1);
    L_nc     = L_nc(-1)^(1-lg)*LT^lg;                                       % climate-free population (phi_m off)
    gL_nc    = L_nc/L_nc(-1) - 1;
	gZ      = gZ_trend + psi_l*(gL - gL_nc) - psi_m*max(M(-1)-M(-2),0);
	[name='climate sensitive population trend']
	L 		= L(-1)^(1-lg)*LT^lg*(1-phi_m*max(M(-1)-M_ref,0));	            % square it for the quadratic version
	gL      = L/L(-1)-1;
...
steady_state_model;	
	...
	[Z,M]   = get_Z(Z0,gZ0,GZ1,L0,LT,lg,gS0,SIG0,GS1,deltapb,tau0,theta2,pb,chi,omega,Dc,Et,M_1750,nu,mc,sigmaC,xi,M0,tf,gamma,alpha,sigmaL,phi_m,M_ref,psi_l,psi_m);
    W_term  = phi_m*max(M-M_ref,0);            % square it for quadratic version
	L		= LT*(1-W_term)^(1/lg);
    L_nc    = LT;
    gL_nc   = 0;
    gL      = 0;
