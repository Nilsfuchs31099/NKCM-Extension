...
for i1 = 1:nn
    subplot(nx,ny,i1)
    idx = strmatch(deblank(varnames(i1,:)),M_.endo_names,'exact');
    y1 = endo_simul_nz0(idx,:);     % net-zero / Paris
    y2 = endo_simul_bau(idx,:);     % laissez-faire / BAU
    y3 = endo_simul_bsl(idx,:);     % estimated middle
    plot(T2,y1,':', 'linewidth',2,'Color',[50,168,82]/255);  hold on;
    plot(T2,y2,'-', 'linewidth',2,'Color',[194,68,43]/255);
    plot(T2,y3,'--','linewidth',2,'Color',[50,96,168]/255);  hold off;
    title([M_.endo_names_long{idx}],'Interpreter','latex')
    xlim([1985 2100]); grid;
    if i1==2, legend('Net-zero (Paris)','Laissez-faire','Estimated middle','Location','best'); end
end
